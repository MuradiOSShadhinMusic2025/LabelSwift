...
class VideoContainer {
    ...
    var playOn: Bool {
        didSet {
            let play = playOn
            let p = player
            let mute = VideoPlayerController.sharedVideoPlayer.mute
            let bitRate = VideoPlayerController.sharedVideoPlayer.preferredPeakBitRate

            p.isMuted = mute
            playerItem.preferredPeakBitRate = bitRate

            DispatchQueue.main.async {
                if play && p.currentItem?.status == .readyToPlay {
                    p.play()
                } else {
                    p.pause()
                }
...
class VideoPlayerController: NSObject, NSCacheDelegate {
    ...
    nonisolated(unsafe) static var playerViewControllerKVOContext = 0
    nonisolated(unsafe) static let sharedVideoPlayer = VideoPlayerController()
    ...
    @MainActor func playVideo(withLayer layer: AVPlayerLayer, url: String) {
        videoURL = url
        currentLayer = layer
    ...
    @MainActor func pauseCurrentVideo() {
    ...
    @MainActor @objc func playerDidFinishPlaying(note: NSNotification) {
    ...
    @MainActor func pausePlayeVideosFor(tableView: UITableView, appEnteredFromBackground: Bool = false) {
    ...
    override func observeValue(forKeyPath keyPath: String?,
        ...
        if keyPath == "status" {
            ...
            let shouldPlay = currentContainer.playOn
            let player = currentContainer.player
            let isMuted = self.mute
            let url = currentContainer.url
                ...
                DispatchQueue.main.async {
                    ...
                    DispatchQueue.main.async {
                        if shouldPlay {
                            ...
                            print("Player is ready to play for URL: \(url)")
                        }
                    }
                }
            case .failed:
