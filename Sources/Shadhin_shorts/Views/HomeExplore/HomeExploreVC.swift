...
class HomeExploreVC: UIViewController, NIBVCProtocol {
...
//                vc.cleanUpCache(excluding: [])
    ...
    private func navStackViewTextColorChange(isBool: Bool) {
        if isBool {
            ...
            self.crossBtn.tintColor = .textColorModeType
            self.forYouBtn.setTitleColor(.textColorModeType, for: .normal)
            self.followingBtn.setTitleColor(.textColorModeType, for: .normal)
            self.exploreBtn.setTitleColor(.textColorModeType, for: .normal)
        }
    }
