...
extension CommentVC {
    ...
    private func setupUI() {
        ...
        UITextView.appearance().tintColor = UIColor.bgBtn
    ...
    private func textViewBorderColorChage(isActive: Bool) {
        UIView.animate(withDuration: 0.3) {
            if isActive {
                self.shadowBgView.backgroundColor = .textViewBg
                self.shadowBgView.layer.borderColor = UIColor.bgBtn.cgColor
                self.shadowBgView.layer.borderWidth = 1
                self.shadowBgView.addShadow(offset: CGSize.init(width: 0, height: 0), color: UIColor.bgBtn, radius: 5.0, opacity: 0.3)
            } else {
                self.shadowBgView.backgroundColor = .textViewBg
                self.shadowBgView.layer.borderColor = UIColor.topBarModeType.withAlphaComponent(0.3).cgColor
                self.shadowBgView.layer.borderWidth = 1
                self.shadowBgView.addShadow(offset: CGSize.init(width: 0, height: 0), color: UIColor.topBarModeType, radius: 5.0, opacity: 0.3)
            }
            self.view.layoutIfNeeded()
        }
    }
